...
def run_cycle(images: dict[str, str], cycle_idx: int) -> bool:
    ...
    try:
       ...
       # run_adb(["shell", "su", "0", "am", "force-stop", "app.greyshirts.firewall"])
        ...
        time.sleep(0.1)
        wait_until_detect_then_delay_click_with_timeout(
            images["RESULT3"], "RESULT3", delay_before_click_sec=0.2, timeout_sec=0.4, click_kwargs=strong_single
        )   
        wait_until_detect_then_delay_click_with_timeout(
            images["RESULT3"], "RESULT3", delay_before_click_sec=0.1, timeout_sec=0.4, click_kwargs=strong_single
        )   
