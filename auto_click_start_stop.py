...
def build_image_map() -> dict[str, str]:
    image_paths = {
        ...
        "WORLDM2": BASE_DIR / "worldeventok.png",
...
def run_cycle(images: dict[str, str], cycle_idx: int) -> bool:
    ...
    try:
        ...
        time.sleep(0.3)
        wait_until_detect_then_delay_click_with_timeout(
            images["WORLDM"], "WORLDM", delay_before_click_sec=0.8, timeout_sec=0.8
        )
        wait_until_detect_then_delay_click_with_timeout(
            images["WORLDM"], "WORLDM", delay_before_click_sec=0.8, timeout_sec=0.8
        )
        ...
        if not wait_until_detect_then_delay_click_with_timeout(
            images["WORLDM2"], "WORLDM2", delay_before_click_sec=0.4, timeout_sec=2.0
        ):
       ...
       # run_adb(["shell", "su", "0", "am", "force-stop", "app.greyshirts.firewall"])
        ...
        time.sleep(0.1)
        wait_until_detect_then_delay_click_with_timeout(
            images["RESULT3"], "RESULT3", delay_before_click_sec=0.1, timeout_sec=0.2, click_kwargs=strong_single
        )   
        wait_until_detect_then_delay_click_with_timeout(
            images["RESULT3"], "RESULT3", delay_before_click_sec=0.1, timeout_sec=0.2, click_kwargs=strong_single
        )   
        ...
        wait_until_detect_then_delay_click_with_timeout(
            images["RESULT"], "RESULT", delay_before_click_sec=0.1, timeout_sec=0.1, click_kwargs=strong_single
        )   
        ...
        def run_gold_tail() -> None:
            ...
            tail = [
               ...
               # ("RESULT", 4.0, strong_single),
                ...
                ("RESULT3", 0.2, strong_single),
                ("RESULT3", 0.2, strong_single),
                ("RESULT4", 0.1, strong_single),
                ("RESULT5", 0.2, strong_single),
                ("RESULT5", 0.2, strong_single),
            ...
            for label, timeout, kwargs in tail:
                ...
                if label == "MAP":
                    ...
                    if label == "RESULT":
                       time.sleep(0.1)
                       wait_until_detect_then_delay_click_with_timeout(
                           images[label], label, delay_before_click_sec=0.2, timeout_sec=2, click_kwargs=kwargs
                       )
                       wait_until_detect_then_delay_click_with_timeout(
        ...
        if not gold_found:
            result_found = wait_until_detect_then_delay_click_with_timeout(
                images["RESULT"], "RESULT-CHECK", delay_before_click_sec=0.1, timeout_sec=0.3, click_kwargs=strong_single
            )
            if result_found:
                print("GOLD not detected, but RESULT detected -> run gold tail path")
                run_gold_tail()
            else:
                ...
                def try_map_with_result3_retries(map_timeout: float) -> None:
                    ...
                    wait_until_detect_then_delay_click_with_timeout(
                        images["MAP"], "MAP", delay_before_click_sec=0.1, timeout_sec=0.3, click_kwargs=strong_single
                    )                    
